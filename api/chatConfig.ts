...
export function buildSystemPrompt(language: string): string {
  return `You are the official AI Representative for "1618 Digital" (a premium digital agency).
...
export function getLocalFallback(message: string, lang: string): string {
  ...
  if (lang === 'de') {
    if (lowercaseMsg.includes('preis') || lowercaseMsg.includes('kosten') || lowercaseMsg.includes('budget')) {
      return "Unsere Preisgestaltung orientiert sich ganz an dem individuellen Umfang Ihres Projekts. Bitte hinterlassen Sie Ihre E-Mail-Adresse im Chat oder schreiben Sie uns direkt an info@1618-digital.de, damit wir Ihnen ein maßgeschneidertes Angebot unterbreiten können.";
    }
  ...
  if (lowercaseMsg.includes('price') || lowercaseMsg.includes('cost') || lowercaseMsg.includes('pricing') || lowercaseMsg.includes('budget')) {
    return "Our pricing is tailored to the exact specifications and complexity of your project. To receive a customized estimate, please leave your email in the header block above or reach out directly to info@1618-digital.de.";
  }
  if (lowercaseMsg.includes('service') || lowercaseMsg.includes('offer') || lowercaseMsg.includes('saas') || lowercaseMsg.includes('ai') || lowercaseMsg.includes('code') || lowercaseMsg.includes('website') || lowercaseMsg.includes('web')) {
    return "1618 Digital specializes in web design & development, high-end 3D SaaS solutions, 24/7 autonomous AI Voice Receptionists, and premium cinematic production (including drone filming and color grading). Please share your email so we can initiate a formal briefing.";
  }
  return "Hello! I am the AI Representative for 1618 Digital. I can tell you all about our websites, premium 3D SaaS systems, custom AI integrations, and cinematic visual productions. To schedule a call with our Director, Ömer Arslaner, please submit your email in the box above or type it directly in our chat.";
}
