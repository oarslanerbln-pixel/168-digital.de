...
## 2026-07-27 - Programmatic Reverse Tabnabbing
**Vulnerability:** Calls to `window.open` lacked the `'noopener,noreferrer'` flags, making them vulnerable to reverse tabnabbing.
**Learning:** React doesn't auto-protect programmatic `window.open` calls like it does for declarative `<a target="_blank">`.
**Prevention:** Always append `'noopener,noreferrer'` as the third argument when programmatically opening external URLs in a new tab.
