...
## 2024-07-24 - Programmatic Reverse Tabnabbing Vulnerability
**Vulnerability:** Programmatic `window.open` calls lacked `'noopener,noreferrer'` protections.
**Learning:** While React automatically protects declarative `<a target="_blank">` tags, it does not auto-protect programmatic calls to `window.open(url, '_blank')`.
**Prevention:** Always explicitly append `'noopener,noreferrer'` as the third argument when using `window.open(url, '_blank')`.
